...
const useCartStore = defineStore("orderCart", () => {
  ...
  const FETCH_ORDERS = async (cartId) => {
    try {
      const apiUrl = "http://10.20.3.163:9002/order/getAllOrdersById";
  ...
  const FETCH_CART = async (cartId) => {
    try {
      ...
      const apiUrl = "http://10.20.3.163:8090/cart/getCartById";
      const queryParams = new URLSearchParams();
      queryParams.set("cartId", cartId);
  ...
  const ADD_TO_CART = async (product, count) => {
    ...
    // console.log(head)
    const res = await fetch("http://10.20.3.163:8090/cart/addToCart", head);
    // console.log(res)
    const parsedResponse = await res.json();
